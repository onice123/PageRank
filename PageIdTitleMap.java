...
final class PageIdTitleMap {
	...
	public static Map<String,Integer> readSqlFile(File file) throws IOException {
		...
		try {
			while (true) {
				...
				for (List<Object> tuple : data) {// data（データ）からtupleとして値を順に取り出す
					if (tuple.size() != 12)
						throw new IllegalArgumentException();// 自分のメソッドの想定外の値が渡されるのは 呼び出し元に問題があるので、処理を続行できない
					
					Object namespace = tuple.get(1);
					if (!(namespace instanceof Integer))// namespaceがIntegerを実装していればtrueが返される
	...
	public static Map<String,Integer> readRawFile(File file) throws IOException {
		...
		try {
			for (int i = 0; ; i++) {
				String line = in.readLine();// テキスト行の読み込み
				if (line == null)
					break;
				if (i >= nextPrint) {
	...
	public static void writeRawFile(Map<String,Integer> idByTitle, File file) throws IOException {
		long startTime = System.currentTimeMillis();
		PrintWriter out = new PrintWriter(new OutputStreamWriter(new BufferedOutputStream(new FileOutputStream(file), 128 * 1024), "UTF-8"));// 文字出力のためのエンコーディング
		try {
			int nextPrint = 0;
			int i = 0;
